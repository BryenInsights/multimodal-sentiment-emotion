"""
Multimodal Video Sentiment Analysis Inference

Provides video, audio, and text processing for emotion/sentiment prediction.
Implements SageMaker-compatible entry points (model_fn, predict_fn, input_fn, output_fn),
with optional local testing.
"""
...
def ensure_ffmpeg(model_dir=None):
    # 1) Already on PATH?
    p = shutil.which("ffmpeg")
    if p:
        return True, p

    # 2) If caller gave us model_dir, check model_dir/bin/ffmpeg
    if model_dir:
        bundled = os.path.join(model_dir, "bin", "ffmpeg")
        if os.path.isfile(bundled) and os.access(bundled, os.X_OK):
            os.environ["PATH"] = (
                os.path.dirname(bundled) + os.pathsep + os.environ.get("PATH", "")
            )
            return True, bundled

    # 3) Fallback: check next to inference.py (code/bin)
    here = os.path.dirname(os.path.abspath(__file__))
    bundled = os.path.join(here, "bin", "ffmpeg")
    if os.path.isfile(bundled) and os.access(bundled, os.X_OK):
        os.environ["PATH"] = (
            os.path.dirname(bundled) + os.pathsep + os.environ.get("PATH", "")
        )
    ...
    return (
        False,
        "FFmpeg not found. Please bundle it in model/bin or code/bin, or use a custom image.",
    )
...
class VideoProcessor:
    def process_video(self, video_path):
        ...
        if len(frames) == 0:
            raise ValueError(f"No valid frames extracted")
...
def output_fn(prediction, response_content_type):
    ...
    raise ValueError("Unsupported content type: " + response_content_type)


def model_fn(model_dir):
    # Ensure ffmpeg is available
    ffmpeg_ok, ffmpeg_info = ensure_ffmpeg(model_dir)
    if not ffmpeg_ok:
        # Instead of crashing the worker, just log and continue
        print("WARNING: FFmpeg not found ->", ffmpeg_info)
    ...
    return {
        ...
        "device": device,
        "ffmpeg_ok": ffmpeg_ok,
        "ffmpeg_info": ffmpeg_info,  # either path or error message
    }


def predict_fn(input_data, model_dict):
    # Check if ffmpeg is available
    if not model_dict.get("ffmpeg_ok", False):
        return {
            "error": "MissingDependency",
            "message": "FFmpeg is required for audio/video processing.",
            "detail": model_dict.get("ffmpeg_info", ""),
            "how_to_fix": [
                "Bundle a static Linux ffmpeg under code/bin/ffmpeg (quick fix).",
                "Or build a custom Docker image with ffmpeg preinstalled.",
            ],
        }
    # Inference code
    ...
    for segment in result["segments"]:
        try:
            ...
            print(f"Segment failed infereance: " + str(e))
        finally:
