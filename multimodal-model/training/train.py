"""
Training script for multimodal sentiment & emotion classification.
- Loads MELD dataset (audio, video, text)
- Trains MultimodalSentimentModel with validation & test evaluation
- Supports AWS SageMaker or local runs
"""
...
def main():
    # Main training workflow: setup, train, validate, save best, and test
    ...
    # Track initial GPU memory usage if GPU is available
    if torch.cuda.is_available():
        torch.cuda.reset_peak_memory_stats()
        memory_used = torch.cuda.max_memory_allocated() / 1024**3
        print(f"Initial GPU memory used: {memory_used:.2f} GB")

    # Prepare dataloaders for train, validation, and test sets
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        train_loss = trainer.train_epoch()  # Train for one epoch
        val_loss, val_metrics = trainer.evaluate(val_loader)  # Validate after epoch
        ...
        # Print metrics in SageMaker-friendly format
        print(
            json.dumps(
                {
                    "metrics": [
        ...
        # Show peak GPU memory usage after each epoch
        if torch.cuda.is_available():
            memory_used = torch.cuda.max_memory_allocated() / 1024**3
    ...
    # After training, evaluate best model on the test set
